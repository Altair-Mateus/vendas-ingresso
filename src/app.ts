...
app.use(async (req, res, next) =>{
    ...
    const isUnprotectedRout = unprotectedRoutes.some( 
...
app.post("/customers/register", async (req, res) => {
    ...
    try {
        ...
        res
            .status(201)
            .json({id: costumerResult.insertId, userId, address, phone, createdAt});
...
app.post("/partners/events", async (req, res) => {
    ...
    res.status(201).json({
        id: eventResult.insertId, name, description, date: eventDate, location, created_at: createdAt, partner_id: partner.id
    })
} finally {
    await connection.end();
}
...
app.get("/partners/events/:eventId", async (req, res) => {
    ...
    try{
        ...
        const event = eventRows.length ? eventRows : null;
