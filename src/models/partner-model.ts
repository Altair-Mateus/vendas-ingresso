...
export class PartnerModel{
    ...
    static async findById(
        ...
        if(options?.user){
            query =
                " SELECT p*, users.id as user_id, users.name as user_name, users.email as user_email " + 
                " FROM partners p JOIN users ON p.user_id = users.id WHERE p.id = ?"
        }
    ...
    static async findByUserId(
        ...
        if(options?.user){
            query =
                " SELECT p*, users.id as user_id, users.name as user_name, users.email as user_email " + 
                " FROM partners p JOIN users ON p.user_id = users.id WHERE p.user_id = ?"
        }
