...
app.use((req, res, next) => __awaiter(void 0, void 0, void 0, function* () {
    var _a;
    const isUnprotectedRout = unprotectedRoutes.some(route => route.method == req.method && req.path.startsWith(route.path));
    if (isUnprotectedRout) {
        return next();
    }
    ...
    try {
        ...
        const user = yield userService.findByUserId(payload.id);
        if (!user) {
            res.status(401).json({ message: "Failed to authentication token" });
        }
        req.user = user;
        next();
    }
...
app.use('/purchases', purchase_controller_1.purchaseRoutes);
app.listen(3000, () => __awaiter(void 0, void 0, void 0, function* () {
    const connection = yield database_1.Database.getInstance();
